...
Definition geometryInterface__mset : list (string * val) := [
  ("Square", geometryInterface__Square);
  ("Volume", geometryInterface__Volume)
].

Definition geometryInterface__mset_ptr : list (string * val) := [
].
...
Definition measureArea : val :=
  rec: "measureArea" "t" :=
    ...
    return: ((interface.get #(str "Square") (![geometryInterface] "t")) #());;;
    do:  #()).
...
Definition measureVolumePlusNM : val :=
  rec: "measureVolumePlusNM" "t" "n" "m" :=
    ...
    return: ((((interface.get #(str "Volume") (![geometryInterface] "t")) #()) + (![uint64T] "n")) + (![uint64T] "m"));;;
    do:  #()).
...
Definition measureVolume : val :=
  rec: "measureVolume" "t" :=
    exception_do (let: "t" := ref_ty geometryInterface "t" in
    return: ((interface.get #(str "Volume") (![geometryInterface] "t")) #());;;
    do:  #()).
...
Definition testMapSize : val :=
  rec: "testMapSize" <> :=
    ...
    let: "$r0" := map.make uint64T uint64T #() in
    do:  ("m" <-[mapT uint64T uint64T] "$r0");;;
    let: "$r0" := (![boolT] "ok") && ((MapLen (![mapT uint64T uint64T] "m")) = #0) in
    do:  ("ok" <-[boolT] "$r0");;;
    let: "$r0" := #1 in
    do:  (map.insert (![mapT uint64T uint64T] "m") #0 "$r0");;;
    let: "$r0" := #2 in
    ...
    let: "$r0" := #4 in
    do:  (map.insert (![mapT uint64T uint64T] "m") #3 "$r0");;;
    let: "$r0" := (![boolT] "ok") && ((MapLen (![mapT uint64T uint64T] "m")) = #3) in
    do:  ("ok" <-[boolT] "$r0");;;
    return: (![boolT] "ok");;;
    do:  #()).
...
Definition testSliceOps : val :=
  rec: "testSliceOps" <> :=
    ...
    let: "$r0" := SliceRef uint64T (![sliceT uint64T] "x") #2 in
    do:  ("v4" <-[ptrT] "$r0");;;
    let: "ok" := ref_ty boolT #true in
...
Definition stringAppend : val :=
  rec: "stringAppend" "s" "x" :=
    ...
    return: ((![stringT] "s") + (let: "$a0" := ![uint64T] "x" in
     machine.UInt64ToString "$a0"));;;
    do:  #()).
...
Definition failing_testStringAppend : val :=
  rec: "failing_testStringAppend" <> :=
    ...
    let: "y" := ref_ty stringT (let: "$a0" := ![stringT] "s" in
    let: "$a1" := #45 in
    stringAppend "$a0" "$a1") in
...
Definition failing_testStringLength : val :=
  rec: "failing_testStringLength" <> :=
    ...
    let: "$r0" := (![boolT] "ok") && ((StringLength (![stringT] "s")) = #0) in
    do:  ("ok" <-[boolT] "$r0");;;
    let: "$r0" := let: "$a0" := ![stringT] "s" in
    let: "$a1" := #1 in
    stringAppend "$a0" "$a1" in
    do:  ("s" <-[stringT] "$r0");;;
    let: "$r0" := (![boolT] "ok") && ((StringLength (![stringT] "s")) = #1) in
    do:  ("ok" <-[boolT] "$r0");;;
    let: "$r0" := let: "$a0" := ![stringT] "s" in
    let: "$a1" := #23 in
    stringAppend "$a0" "$a1" in
...
Definition testStructUpdates : val :=
  rec: "testStructUpdates" <> :=
    ...
    let: "$r0" := (![boolT] "ok") && ((struct.get TwoInts "x" ((S__readBVal (![ptrT] "ns")) #())) = #4) in
    do:  ("ok" <-[boolT] "$r0");;;
    return: (![boolT] "ok");;;
    do:  #()).
...
Definition Log__Commit : val :=
  rec: "Log__Commit" "l" <> :=
    ...
    do:  (let: "$a0" := #0 in
    let: "$a1" := ![sliceT byteT] "header" in
    (interface.get #(str "Write") (![disk.Disk] (struct.field_ref Log "d" "l"))) "$a0" "$a1");;;
    do:  #()).
...
Definition Log__Read : val :=
  rec: "Log__Read" "l" "a" :=
    ...
    (interface.get #(str "Read") (![disk.Disk] (struct.field_ref Log "d" "l"))) "$a0" in
...
Definition Log__Size : val :=
  rec: "Log__Size" "l" <> :=
    ...
    let: "$r0" := (interface.get #(str "Size") (![disk.Disk] (struct.field_ref Log "d" "l"))) #() in
    do:  ("sz" <-[uint64T] "$r0");;;
...
Definition New : val :=
  rec: "New" <> :=
    ...
    let: "$r0" := (interface.get #(str "Size") (![disk.Disk] "d")) #() in
...
Definition disabled_testWal : val :=
  rec: "disabled_testWal" <> :=
    ...
    (interface.get #(str "Read") (![disk.Disk] (struct.field_ref Log "d" "lg"))) "$a0" in
    blockToInt "$a0") = #0) in
    do:  ("ok" <-[boolT] "$r0");;;
