...
Definition testContinueFromLoop: val :=
  rec: "testContinueFromLoop" <> :=
    let: "i" := ref_to uint64T #0 in
    Skip;;
    (for: (λ: <>, ![uint64T] "i" < #3); (λ: <>, Skip) := λ: <>,
      (if: #true
      ...
      Continue);;
    (![uint64T] "i" = #1).
Theorem testContinueFromLoop_t: ⊢ testContinueFromLoop : (unitT -> boolT).
Proof. typecheck. Qed.
Hint Resolve testContinueFromLoop_t : types.
